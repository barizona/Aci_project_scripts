...
args <- list(target = "city_pooled_new",
...
tree_tbl <- read_tsv("output/city_pooled_filled.tsv") %>% 
...
for (i in 1:length(args$heatmap_vars)) {
  ...
  if (args$heatmap_vars[i] == args$target) {
    ...
    names(cls[[i]]) <- geo_cols$city_pooled_new
  } else {
    set.seed(0)
...
ggsave(
  file = "output/dated_tree_with_state_changes_new.pdf",
