...
# AIM: Survival plot of PBS, Aci 110 (wt) and A110-2, Figure 7B ----------------
#xxxxxxxxxxxxxxxxxxxxx
...
B_tab <- read_tsv("input/Fig7_Exp_20230911_19.tsv") %>% 
...
#xxxxxxx
B_log_rank_test <- pairwise_survdiff(Surv(
    time = Time, event = Event) ~ Strain, data = B_tab, 
    rho = 0, p.adjust.method = "none")

# convert to stars indicating the level of significance
B_signif <- B_log_rank_test$p.value[var_aci,] %>% 
    as_tibble() %>% 
...
B_start_end <- p_B_v1$data.survplot %>% 
...
names(B_start_end) <- c("Aci 110", var_aci)
...
p_B <- p_B_v1$plot +
    ...
    annotate("segment", x = 51, xend = 51, 
             y = B_start_end["Aci 110"], yend = B_start_end[2], 
             color = "black", linewidth = 0.3, lineend = "round") +
    annotate("segment", x = 50, xend = 51, 
             y = B_start_end["Aci 110"], yend = B_start_end["Aci 110"], 
             color = "black", linewidth = 0.3, lineend = "round") +
    annotate("segment", x = 50, xend = 51, 
             y = B_start_end[2], yend = B_start_end[2], 
             color = "black", linewidth = 0.3, lineend = "round") +
    annotate("text", x = 53, y = mean(c(B_start_end["Aci 110"], B_start_end[2])), 
             angle = 90, label = B_signif["Aci 110"], size = 5, 
             hjust = 0.5, vjust = 0.5) +
    theme(# legend
...
rm(var_aci, B_signif, B_start_end, p_B_v1)
