...
composit_ABC <- plot_grid(NULL, p_A, NULL, p_B, NULL, p_C,
                          labels = c("A", "", "B", "", "C",""), 
                          label_fontface = "plain", label_size = 12, 
                          label_fontfamily = "Arial",
                          nrow = 1, align = "h", axis = "l", 
                          rel_widths = c(0.07, 1, 0.1, 1, 0.1, 1)) %>% 
    plot_grid(p_ABClegend, NULL, ., ncol = 1, rel_heights = c(0.1, 0.01, 0.9))

composit_DEF <- plot_grid(NULL, p_D, NULL, p_E, NULL, p_F,
                          labels = c("D", "", "", "E", "", "F"), 
                          label_fontface = "plain", label_size = 12,
                          label_fontfamily = "Arial",
                          nrow = 1, 
                          align = "h", axis = "t",
                          rel_widths = c(0.12, 0.45, 0.05, 1, 0.05, 1))


p <- plot_grid(composit_ABC, composit_DEF, ncol = 1, rel_heights = c(0.8, 1))

# Each figure should fit on a single 8.5” x 11” page
ggsave("output/Figure7_orig.pdf", p, units = "in", height = 6, width = 8.5, 
       device = cairo_pdf)

# with Inkscape move the superscript R to the right place in F and save as Figure7.pdf
