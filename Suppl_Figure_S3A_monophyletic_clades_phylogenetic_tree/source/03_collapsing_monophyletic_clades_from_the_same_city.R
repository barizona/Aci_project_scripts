...
# clades of the 16 nodes
clade_16_node <- tree_tbl %>% 
  filter(node %in% nodes) %>% 
  select(label) %>% 
  pull()
clade_16_node <- tree_tbl_collapsed %>% 
  filter(label %in% clade_16_node) %>% 
  select(node, label) %>% 
  deframe()

p_collapsed_fan_v3 <- p_collapsed_fan_v2 +
  geom_cladelab(node = as.numeric(names(clade_16_node)), label = clade_16_node, 
                geom = "text", offset = -10, hjust = 0.5, vjust = 0.5, color = "black")

ggsave(file = "output/dated_tree_with_state_changes_collapsed_fan_with_10_clades.pdf", p_collapsed_fan_v3,
       height = 0.07*nrow(tree_tbl_collapsed),
       width = 0.06*nrow(tree_tbl_collapsed),
       units = "cm", limitsize = FALSE)
